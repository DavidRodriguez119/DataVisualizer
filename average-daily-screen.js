...
function AverageDailyScreen() {
  ...
  this.draw = function () {
      ...
      this.drawGradientBackground(); // Calling the class method
  ...
  // ----------------------------------------
  // Helper: Draw a subtle vertical gradient (CLASS METHOD NOW)
  // ----------------------------------------
  this.drawGradientBackground = function() { // Changed to class method
  ...
  // ----------------------------------------
    // DESTROY (CLEANUP WHEN SWITCHING VISUALS) - LESS Aggressive Reset
    // ----------------------------------------
    // Remove UI elements, reset flags, clear *some* data, and *reset p5.js states*
    this.destroy = function () {
      console.log("AverageDailyScreen destroy() called - Less Aggressive Reset"); // Updated console log

      // 1. Remove UI elements (as before)
      if (this.yearSlider) {
      ...
      this.uiInitialized = false; // Keep resetting uiInitialized to allow setup on re-entry
      ...
      // 3. **Aggressively Reset p5.js Drawing States** (as before)
      // Reset text styles
      textSize(16); // Reset to a common default text size (adjust if needed)
      textFont('Arial'); // Or whatever your default font is
      textAlign(LEFT, TOP); // Reset to default text alignment

      // Reset shape styles
      rectMode(CORNER); // Reset rectMode to default CORNER
      ellipseMode(CENTER); // Reset ellipseMode to default CENTER (though not used in this code, good practice)

      // Reset color and stroke
      fill(0);       // Reset fill color to black (or your default)
      stroke(0);     // Reset stroke color to black (or your default)
      strokeWeight(1); // Reset stroke weight to 1 (or your default)
      noStroke();     // if noStroke is your default for fills, uncomment this, otherwise, keep `stroke(0)` if you expect strokes to be default. Choose based on your typical style.

      colorMode(RGB, 255); // Reset color mode to RGB, 255 range (common default)

      // Reset cursor
      cursor(ARROW);   // Reset cursor back to default arrow

      // Reset angle mode (if you use angles)
      angleMode(RADIANS); // Or angleMode(DEGREES) if your project defaults to degrees.
  };
};

// The global function definition has been removed!
// function drawGradientBackground() { ... } // DELETE THIS LINE
